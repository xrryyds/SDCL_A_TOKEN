...
def train_on_MATH_4(epoch: int = 1):
    ...
    train_a_token_sd_api_4(
        ...
        learning_rate=1e-6,
        max_new_tokens=4096,
        gradient_accumulation_steps=8,   # 与双卡版一致，梯度累积平滑
        rollout_batch_size=16,           # 与双卡版一致，避免显存压力过大
        vllm_gpu_memory_utilization=0.85, # 降回 0.85，避免 KV cache 不足截断生成
        alpha=0.1,
        delta=0.1,
        vllm_tensor_parallel_size=4,     # 唯一改动：4 卡 TP，rollout 速度 ~3.5x
        gradient_checkpointing=False,    # H200 140GB 可关闭，提升训练速度
        save_total_limit=1,
        device="cuda:0",
    )
