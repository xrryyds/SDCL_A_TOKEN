...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        self_distillation_needs_ref = self_distillation_cfg is not None and loss_mode in ("sdpo", "srpo")
        if self_distillation_needs_ref and need_reference_policy(config):
            raise ValueError("SDPO/SRPO cannot share the reference policy with KL regularization.")
        if self_distillation_needs_ref and use_legacy_worker_impl == "disable":
            raise ValueError(
                "SDPO/SRPO requires the legacy worker implementation to colocate the teacher."
            )
