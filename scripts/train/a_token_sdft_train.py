"""SDFT 路线训练脚本 (off-policy KD + prompt asymmetry)。

设计 (用户 2026-05-31 拍板):
  - 数据复用 V3 三池: train_data_v3.json (corr 5456 + roll 2134 + pool 24326 = 31916)
  - corr_answer : teacher prompt == student prompt (正常),全 span 反向 KL(s‖t)
  - roll / pool : teacher 加 hint "Please start your answer with \"{token_text}\". {q}"
                 student 用正常 prompt
                 用采集到的 candidate.answer 作为共同 answer span
                 全 span 反向 KL(s‖t) 在 answer span 上对齐计算
  - 加权: 全 w=1.0 纯 sum, 不归一化 (同 V3)
  - 2 卡 DDP, 2 epoch, max_prompt=2048, max_answer=4096, LoRA r=32 alpha=64

关键实现:
  teacher / student forward 序列长度不同 (teacher 多 hint),
  但 answer span 长度相同 (都是 candidate.answer 的 token 数)。
  分别取各自 answer span 的 logits 做 KL,token 一一对应。

不擅作主张过的设计点:
  - hint 用英文 (A 方案): f"Please start your answer with \"{ttxt}\". {q}"
  - corr 不用 hint (teacher=student prompt,等价于纯反向 KL,与 V3 一致)
"""
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="SDFT 路线训练 (off-policy KD + prompt asymmetry)"
    )
