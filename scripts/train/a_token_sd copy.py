...
def _extract_first_completion_token_ids_batch(
    ...
    """批量提取每条 rollout completion 的首 token id，减少 Python/tokenizer 调用开销。"""
    flat_texts: List[str] = []
    group_sizes: List[int] = []
    for prompt_text, rollout_seqs in zip(prompt_texts, rollout_groups):
        group_sizes.append(len(rollout_seqs))
        flat_texts.extend(prompt_text + seq for seq in rollout_seqs)
...
def train_a_token_sd(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i in mistake_progress:
            ...
            batch_first_token_ids = _extract_first_completion_token_ids_batch(
                tokenizer,
                batch_prompts,
                batch_rollouts,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="训练 batch size（4x140GB H200 推荐 64~128）",
    )
