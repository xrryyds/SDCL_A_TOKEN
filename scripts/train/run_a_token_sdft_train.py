"""DDP launcher for a_token_sdft_train.py (SDFT 路线)。
...
用法:
    CUDA_VISIBLE_DEVICES=0,1 python scripts/train/run_a_token_sdft_train.py \
        ...
        --output_dir output/sdft_v1_$(date +%Y%m%d_%H%M%S) \
        --num_epochs 2 ...

进程数自动 = torch.cuda.device_count();可用 --nproc 显式覆盖。
未识别参数原样转发给 a_token_sdft_train.py。
...
_TRAIN_SCRIPT = os.path.join(_THIS_DIR, "a_token_sdft_train.py")
...
def _launch_training():
    parser = argparse.ArgumentParser(
        description="SDFT 训练 DDP launcher", add_help=True,
    )
    parser.add_argument("--nproc", type=int, default=None)
    parser.add_argument(
        "--master_port", type=str,
        default=os.environ.get("MASTER_PORT", "29501"),  # SDFT 用 29501,避免与 V3 训练冲突
    )
    args, forwarded = parser.parse_known_args()

    nproc = _decide_nproc(args.nproc)
    print(
        f"[run_a_token_sdft_train] world_size={nproc}, "
    ...
    # GPU 健康检查
    print("[run_a_token_sdft_train] 健康检查 GPU init...", flush=True)
    ...
    print("[run_a_token_sdft_train] GPU 健康检查通过。", flush=True)
