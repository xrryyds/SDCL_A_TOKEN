"""Build fill pool by exhaustive first-token池: 对 unresolved 池每题,
...
unresolved 池连续切片分发给各 worker;主进程 Queue 收集 + 合并 + 写盘。

流程 (per worker):
  shard = 该 worker 分到的 unresolved 题
...
输出 (完全替换):
  ...
  datasets/exam/fill_multi_unresolved.json  # 744 - N 题仍未救回
...
DEFAULT_UNRESOLVED_PATH = os.path.join(
    _PROJECT_ROOT, "datasets", "exam", "fill_multi_unresolved_roll.json"
)
...
def main():
    ...
    parser.add_argument(
        ...
        help="输入: 上一阶段 fill_multi 没救回的题 (744 题)。"
    )
    parser.add_argument(
