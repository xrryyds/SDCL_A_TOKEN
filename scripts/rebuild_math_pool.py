"""一次性脚本：用 2048+4096 干净口径重建 mistake / corr 池。

Step A 全流程：
  1. 备份旧 mistake_DS_MATH_pool.json / corr_DS_MATH_pool.json
  2. take_exam (MATH train, max_prompt=6144=2048+4096, max_new=4096) → datasets/exam/exam.json
  3. teacher_mark_paper_with_save() 按 boxed{} 字符串相等拆分
     → datasets/exam/mistake_collection_book_4096.json  (FileIOUtils.mistake_file_path)
     → datasets/exam/corr_answer_4096.json              (FileIOUtils.corr_path)
  4. 复制成 mistake_DS_MATH_pool.json / corr_DS_MATH_pool.json (覆盖旧名)
  5. 跑完或异常都调 use_worker() 挂卡保活

用法（本地 2 卡 H800）：
    cd /workspace/SDCL_A_TOKEN  # 或本地项目根
...
# teacher_mark_paper_with_save 写入的中间文件（FileIOUtils 硬编码）
...
def main():
    print("=" * 70, flush=True)
    print(f"[rebuild_math_pool] start ts={TS}", flush=True)
    print(f"[rebuild_math_pool] project_root={_PROJECT_ROOT}", flush=True)
    print(f"[rebuild_math_pool] CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}", flush=True)
    print("=" * 70, flush=True)
    ...
    # 2) take_exam
    print("\n[step 1/3] take_exam on MATH train (7500 题, max_prompt=6144, max_new=4096) ...", flush=True)
    from main import student_take_exam_Math_sub

    student_take_exam_Math_sub(
        train=True,
        subset="all",
        lora_path=None,           # baseline，无 LoRA
        max_prompt_length=6144,   # = 2048 (prompt budget) + 4096 (gen) 的 vLLM 总窗口
        max_new_tokens=4096,
    )

    # 3) teacher 判分 + 拆池
    print("\n[step 2/3] teacher_mark_paper_with_save (boxed{} 字符串相等判分, 拆 mistake/corr) ...", flush=True)
    ...
    # 4) 复制成新池名
    print("\n[step 3/3] copy → mistake_DS_MATH_pool.json / corr_DS_MATH_pool.json ...", flush=True)
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"[rebuild_math_pool] DONE", flush=True)
